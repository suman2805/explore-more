...
/*function init_map(){
			var myOptions = {
				zoom:zoom,center:new google.maps.Map LatLng(24.9560587,84.01453980000003)
				//,mapTypeId: google.maps.MapTypeId.ROADMAP
				};
				map = new google.maps.Map(document.getElementById('map'), myOptions);marker = new google.maps.Marker({map: map,position: new google.maps.Map LatLng(24.9560587,84.01453980000003)});infowindow = new google.maps.InfoWindow({content:'<strong>sasaram</strong>'});google.maps.event.addListener(marker, 'click', 
				function(){
				infowindow.open(map,marker);});infowindow.open(map,marker);}google.maps.Map event.addDomListener(window, 'load', init_map);
}*/
